...
export function LichessBoard({
  ...
  useEffect(() => {
    ...
    api.current = Chessground(host.current, boardConfig(fen, orientation, lastMove));
    return () => {
      api.current?.destroy();
      api.current = null;
    };
  ...
  useEffect(() => {
    api.current?.set(boardConfig(fen, orientation, lastMove));
  }, [fen, orientation, last, lastMove]);
