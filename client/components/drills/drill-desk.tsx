...
export function DrillDesk({
  ...
  const coachKey = `${insight?.headline ?? ''}|${answer?.headline ?? ''}`;
  const [seenCoach, setSeenCoach] = useState('');
  if (coachKey !== seenCoach) {
    setSeenCoach(coachKey);
    if (insight || answer) setPane('panel');
  }
